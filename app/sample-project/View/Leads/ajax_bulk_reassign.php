<form action="<?= base_url('app/leads/bulk_reassign') ?>" method="post" enctype="multipart/form-data">
    <div class="row g-3">
        <div class="col-lg-2">
            <div class="p-1">
                <label for="tele_caller_id" class="form-label">Re-assign To</label>
        ...
        <div class="col-lg-2">
            <div class="p-1">
                <label for="from_tele_caller_id" class="form-label">Re-assign From</label>
                <select class="form-control" name="from_tele_caller_id" id="from_tele_caller_id" required>
                    <option value="">Select Telecaller</option>
                    <?php foreach ($tele_callers as $tele_caller) { ?>
                        <option value="<?= $tele_caller['id'] ?>"><?= $tele_caller['name'] ?></option>
                    <?php } ?>
...
$(document).ready(function() {
    ...
    // AJAX to fetch leads
    $('#from_tele_caller_id, #lead_source_id, #lead_from_date, #lead_to_date, #lead_status_id').on('change', function() {
        var leadSourceId = $('#lead_source_id').val();
        var leadStatusId = $('#lead_status_id').val();
        var teleCallerId = $('#from_tele_caller_id').val();
        ...
        if (leadSourceId && teleCallerId && leadFromDate && leadToDate && leadStatusId) {
            $.ajax({
                url: '<?= base_url('app/leads/get_leads_by_source_re_assign') ?>',
                type: 'POST',
                data: { 
