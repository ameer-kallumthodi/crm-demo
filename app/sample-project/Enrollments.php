...
class Enrollments extends AppBaseController
{
    private $users_model;
    private $leads_model;
    ...
    public function index(){
        ...
        if (!empty($this->request->getGet('from_date')) && !empty($this->request->getGet('to_date'))){
            ...
            $filter_where['lead_status_id'] = 7;
            $this->data['list_items'] = $this->leads_model->get($filter_where,null,['id','desc'])->getResultArray();
            // log_last_query();
        }else{
            $this->data['list_items'] = $this->leads_model->get(['lead_status_id' => 7],null,['id','desc'])->getResultArray();
        }
        ...
        $this->data['page_title'] = 'Enrollments';
        $this->data['page_name'] = 'Enrollments/index';
    ...
    public function ajax_add(){
        ...
        echo view('App/Enrollments/ajax_add', $this->data);
    }
    ...
    public function ajax_view($id){
        ...
        $leads = $this->leads_model->get()->getResultArray();
        $this->data['leads_name'] = array_column($leads, 'title', 'id');
