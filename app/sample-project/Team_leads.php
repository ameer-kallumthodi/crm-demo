...
class Team_leads extends AppBaseController
{
    private $users_model;
    ...
    public function index(){
        $this->data['list_items'] = $this->users_model->get(['role_id' => 6, 'is_team_lead' => 1])->getResultArray();
        $this->data['page_title'] = 'Team Leads';
        $this->data['page_name'] = 'Team_leads/index';
        return view('App/index', $this->data);
    }
    
    public function ajax_add(){
        $this->data['country_code'] = get_country_code();
        echo view('App/Team_leads/ajax_add', $this->data);
    }
    ...
    public function add(){
        if ($this->request->getMethod() === 'post'){
            ...
            if ($admin_id){
                session()->setFlashdata('message_success', "Team Lead Added Successfully!");
            }else{
        ...
        return redirect()->to(base_url('app/team_leads/index'));
    }
    
    public function ajax_edit($id){
        $this->data['country_code'] = get_country_code();
        $this->data['edit_data'] = $this->users_model->get(['id' => $id])->getRowArray();
        echo view('App/Team_leads/ajax_edit', $this->data);
    }
    ...
    public function edit($id){
        if ($this->request->getMethod() === 'post'){
            ...
            if ($response){
                session()->setFlashdata('message_success', "Team Lead Updated Successfully!");
            }else{
                session()->setFlashdata('message_danger', "Something went wrong! Try Again");
            }
        }else{
    ...
    public function ajax_view($id){
        ...
        echo view('App/Team_leads/ajax_view', $this->data);
    }
    
    public function delete($id){
        if ($id > 0){
            if ($this->users_model->remove(['id' => $id])){
                session()->setFlashdata('message_success', "Team Lead Deleted Successfully!");
            }else{
                session()->setFlashdata('message_danger', "Something went wrong! Try Again");
            }
        }else{
