...
class Accountants extends AppBaseController
{
    private $users_model;
    ...
    public function index(){
        $this->data['list_items'] = $this->users_model->get(['role_id' => 10])->getResultArray();
        $this->data['page_title'] = 'Accountants';
        $this->data['page_name'] = 'Accountants/index';
        return view('App/index', $this->data);
    }
    
    public function ajax_add(){
        $this->data['country_code'] = get_country_code();
        echo view('App/Accountants/ajax_add', $this->data);
    }
    ...
    public function add(){
        if ($this->request->getMethod() === 'post'){
            ...
            if($check_phone_duplication == 0 && $check_email_duplication == 0){
                ...
                if ($admin_id){
                    session()->setFlashdata('message_success', "Accountant Added Successfully!");
                }else{
        ...
        return redirect()->to(base_url('app/accountants/index'));
    }
    
    public function ajax_edit($id){
        $this->data['country_code'] = get_country_code();
        $this->data['edit_data'] = $this->users_model->get(['id' => $id])->getRowArray();
        echo view('App/Accountants/ajax_edit', $this->data);
    }
    ...
    public function edit($id){
        if ($this->request->getMethod() === 'post'){
            ...
            if($check_phone_duplication == 0 && $check_email_duplication == 0){
                ...
                if ($response){
                    session()->setFlashdata('message_success', "Accountant Updated Successfully!");
                }else{
                    session()->setFlashdata('message_danger', "Something went wrong! Try Again");
                }
            }else{
    ...
    public function ajax_view($id){
        ...
        echo view('App/Accountants/ajax_view', $this->data);
    }
    
    public function delete($id){
        if ($id > 0){
            if ($this->users_model->remove(['id' => $id])){
                session()->setFlashdata('message_success', "Accountant Deleted Successfully!");
            }else{
                session()->setFlashdata('message_danger', "Something went wrong! Try Again");
            }
        }else{
