...
class ESchoolEduthanzeelFacultyController extends Controller
{
    /**
     * Display a listing of E-School converted leads for faculty
     */
    public function eschoolIndex(Request $request)
    {
        return $this->getFacultyIndex($request, 5, 'E-School');
    }

    /**
     * Display a listing of Eduthanzeel converted leads for faculty
     */
    public function eduthanzeelIndex(Request $request)
    {
        return $this->getFacultyIndex($request, 6, 'Eduthanzeel');
    }

    /**
     * Common method to get mentor index
     */
    private function getFacultyIndex(Request $request, $courseId, $courseName)
    {
        // Check permissions
        ...
        $viewName = $courseId == 5 ? 'admin.converted-leads.eschool-faculty-index' : 'admin.converted-leads.eduthanzeel-faculty-index';
        $routeName = $courseId == 5 ? 'admin.faculty-eschool-converted-leads.index' : 'admin.faculty-eduthanzeel-converted-leads.index';
    ...
    {
        // Check permissions
        if (!RoleHelper::is_admin_or_super_admin() && !RoleHelper::is_admission_counsellor() && !RoleHelper::is_faculty() && !RoleHelper::is_telecaller() && !RoleHelper::is_team_lead() && !RoleHelper::is_senior_manager() && !RoleHelper::is_general_manager()) {
