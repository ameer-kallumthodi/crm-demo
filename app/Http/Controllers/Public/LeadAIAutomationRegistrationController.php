...
class LeadAIAutomationRegistrationController extends Controller
{
    public function showAIAutomationForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            if ($lead && $lead->aiAutomationStudentDetails) {
                return view('public.ai-automation-registration-success');
            }
        }
        
        // Get AI Automation course subjects (course_id = 12)
        $subjects = Subject::where('course_id', 12)->where('is_active', true)->get();
        
        // Get AI Automation course batches (course_id = 12)
        $batches = Batch::where('course_id', 12)->where('is_active', true)->get();
        
        // Get country codes
        $countryCodes = \App\Helpers\CountriesHelper::get_country_code();
        
        return view('public.ai-automation-registration', compact('subjects', 'batches', 'lead', 'countryCodes'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $studentDetail = LeadDetail::create([
                ...
                'course_id' => 12, // AI Automation course ID
            ...
            try {
                MailService::sendStudentRegistrationEmail($studentDetail, 'AI Automation');
            } catch (\Exception $e) {
                // Log error but don't fail the registration
                \Log::error('Email sending failed for AI Automation registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.ai-automation.register', $request->lead_id)
            ]);
