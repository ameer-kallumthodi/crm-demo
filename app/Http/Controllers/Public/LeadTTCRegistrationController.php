...
class LeadTTCRegistrationController extends Controller
{
    public function showTTCForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            if ($lead && $lead->ttcStudentDetails) {
                return view('public.ttc-registration-success');
            }
        }
        
        // Get TTC course subjects (course_id = 7)
        $subjects = Subject::where('course_id', 7)->where('is_active', true)->get();
        
        // Get TTC course batches (course_id = 7)
        $batches = Batch::where('course_id', 7)->where('is_active', true)->get();
        
        // Get country codes
        $countryCodes = \App\Helpers\CountriesHelper::get_country_code();
        
        return view('public.ttc-registration', compact('subjects', 'batches', 'lead', 'countryCodes'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'plustwo_certificate' => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:2048',
        ...
        try {
            ...
            try {
                MailService::sendStudentRegistrationEmail($studentDetail, 'TTC');
            } catch (\Exception $e) {
                // Log error but don't fail the registration
                \Log::error('Email sending failed for TTC registration: ' . $e->getMessage());
            }
            ...
            try {
                ...
                // Log error but don't fail the registration
                \Log::error('Failed to create lead activity for TTC registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.ttc.register.success', $request->lead_id)
            ]);
