...
class LeadMachineLearningRegistrationController extends Controller
{
    public function showMachineLearningForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            // Check if student has already registered (course_id = 20 for Diploma in Machine Learning)
            if ($lead) {
                ...
                if ($studentDetail) {
                    return view('public.machine-learning-registration-success');
                }
            }
        }
        
        // Get Diploma in Machine Learning course subjects (course_id = 20)
        $subjects = Subject::where('course_id', 20)->where('is_active', true)->get();
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'plus_two_certificate.required' => 'Plus Two certificate is required.',
        ...
        try {
            ...
            $studentDetail = LeadDetail::create([
                ...
                'course_id' => 20, // Diploma in Machine Learning course ID
            ...
            try {
                MailService::sendStudentRegistrationEmail($studentDetail, 'Diploma in Machine Learning');
            } catch (\Exception $e) {
                // Log error but don't fail the registration
                \Log::error('Email sending failed for Diploma in Machine Learning registration: ' . $e->getMessage());
            }
            ...
            try {
                ...
                // Log error but don't fail the registration
                \Log::error('Failed to create lead activity for Diploma in Machine Learning registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.machine-learning.register', $request->lead_id)
            ]);
