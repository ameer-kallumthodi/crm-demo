...
class LeadPythonRegistrationController extends Controller
{
    public function showPythonForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            // Check if student has already registered (course_id = 10 for Python)
            if ($lead) {
                ...
                if ($studentDetail) {
                    return view('public.python-registration-success');
                }
            }
        }
        
        // Get Python course subjects (course_id = 10)
        $subjects = Subject::where('course_id', 10)->where('is_active', true)->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                // Log error but don't fail the registration
                \Log::error('Failed to create lead activity for Python registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.python.register', $request->lead_id)
            ]);
