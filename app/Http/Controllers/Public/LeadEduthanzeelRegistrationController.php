...
class LeadEduthanzeelRegistrationController extends Controller
{
    public function showEduthanzeelForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            if ($lead && $lead->eduthanzeelStudentDetails) {
                return view('public.eduthanzeel-registration-success');
            }
        }
        
        // Get Eduthanzeel course subjects (course_id = 6)
        $subjects = Subject::where('course_id', 6)->where('is_active', true)->get();
        
        // Get Eduthanzeel course batches (course_id = 6)
        $batches = Batch::where('course_id', 6)->where('is_active', true)->get();
        
        // Get country codes
        $countryCodes = \App\Helpers\CountriesHelper::get_country_code();
        
        return view('public.eduthanzeel-registration', compact('subjects', 'batches', 'lead', 'countryCodes'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $studentDetail = LeadDetail::create([
                ...
                'course_id' => 6, // Eduthanzeel course ID
            ...
            try {
                MailService::sendStudentRegistrationEmail($studentDetail, 'Eduthanzeel');
            } catch (\Exception $e) {
                // Log error but don't fail the registration
                \Log::error('Email sending failed for Eduthanzeel registration: ' . $e->getMessage());
            }
            ...
            try {
                ...
                // Log error but don't fail the registration
                \Log::error('Failed to create lead activity for Eduthanzeel registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.eduthanzeel.register.success', $request->lead_id)
            ]);
