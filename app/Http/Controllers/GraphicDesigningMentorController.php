...
class GraphicDesigningMentorController extends Controller
{
    /**
     * Display a listing of Graphic Designing converted leads for mentoring
     */
    public function index(Request $request)
    {
        ...
        if ($currentUser) {
            if (RoleHelper::is_admin_or_super_admin()) {
                ...
                // Check if current course (15) is in HOD's assigned courses
                if (!empty($hodCourseIds) && in_array(15, $hodCourseIds)) {
        ...
        // Get filter data
        $batches = Batch::where('course_id', 15)->orderBy('title')->get();
        ...
        return view('admin.converted-leads.graphic-designing-mentor-index', compact(
            'convertedLeads', 
            'batches', 
            'country_codes',
            'course',
    ...
    public function updateMentorDetails(Request $request, $id)
    {
        try {
            ...
            return response()->json([
                ...
                Log::error('Error updating graphic designing mentor details: ' . $e->getMessage());
            return response()->json([
                'success' => false,
