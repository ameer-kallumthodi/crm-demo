...
class PostSalesReportController extends Controller
{
    ...
    /**
     * Export Post Sales Month Ways Report to PDF
     */
    public function exportPostSalesMonthWaysPdf(Request $request)
    {
        $this->checkAccess();
        ...
        $pdf = Pdf::loadView('admin.reports.exports.post-sales-month-ways-pdf', [
            'reports' => $reports,
            'fromDate' => $fromDateStr,
            'toDate' => $toDateStr,
            'generatedAt' => now()->format('Y-m-d H:i:s')
        ...
        $pdf->setPaper('A4', 'landscape');
        return $pdf->download('post-sales-month-ways-report-' . $fromDateStr . '-to-' . $toDateStr . '.pdf');
    }

    /**
     * Export Total Monthly Report to PDF
     */
    public function exportTotalMonthlyPdf(Request $request)
    {
        $this->checkAccess();
        ...
        $pdf = Pdf::loadView('admin.reports.exports.total-monthly-pdf', [
            'reportData' => $reportData,
            'grandTotal' => $grandTotal,
            'grandTotalStudents' => $grandTotalStudents,
            'fromDate' => $fromDateStr,
        ...
        $pdf->setPaper('A4', 'landscape');
        return $pdf->download('total-monthly-report-' . $fromDateStr . '-to-' . $toDateStr . '.pdf');
    }

    /**
     * Export BDE Collected Amount Course Ways Report to PDF
     */
    public function exportBdeCollectedAmountCourseWaysPdf(Request $request)
    {
        $this->checkAccess();
        ...
        $pdf = Pdf::loadView('admin.reports.exports.bde-collected-amount-course-ways-pdf', [
            'reportData' => $reportData,
            'grandTotal' => $grandTotal,
            'grandTotalStudents' => $grandTotalStudents,
        ...
        $pdf->setPaper('A4', 'landscape');
        return $pdf->download('bde-collected-amount-course-ways-report-' . ($selectedUser ? $selectedUser->name : 'all') . '-' . ($fromDateInput ?: 'all-dates') . '.pdf');
    }
}
