...
class LeadFlutterRegistrationController extends Controller
{
    public function showFlutterForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            // Check if student has already registered (course_id = 21 for Flutter)
            if ($lead) {
                ...
                if ($studentDetail) {
                    return view('public.flutter-registration-success');
                }
            }
        }
        
        // Get Flutter course subjects (course_id = 21)
        $subjects = Subject::where('course_id', 21)->where('is_active', true)->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $studentDetail = LeadDetail::create([
                ...
                'course_id' => 21, // Flutter course ID
            ...
            try {
                MailService::sendStudentRegistrationEmail($studentDetail, 'Flutter');
            } catch (\Exception $e) {
                // Log error but don't fail the registration
                \Log::error('Email sending failed for Flutter registration: ' . $e->getMessage());
            }
            ...
            try {
                ...
                // Log error but don't fail the registration
                \Log::error('Failed to create lead activity for Flutter registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.flutter.register', $request->lead_id)
            ]);
