...
class LeadHotelMgmtRegistrationController extends Controller
{
    public function showHotelMgmtForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            if ($lead && $lead->hotelMgmtStudentDetails) {
                return view('public.hotel-mgmt-registration-success');
            }
        }
        
        // Get Hotel Management course subjects (course_id = 8)
        $subjects = Subject::where('course_id', 8)->where('is_active', true)->get();
        
        // Get Hotel Management course batches (course_id = 8)
        $batches = Batch::where('course_id', 8)->where('is_active', true)->get();
        
        // Get country codes
        $countryCodes = \App\Helpers\CountriesHelper::get_country_code();
        
        return view('public.hotel-mgmt-registration', compact('subjects', 'batches', 'lead', 'countryCodes'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'plustwo_certificate' => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:1024',
        ...
        try {
            // Handle file uploads
            $filePaths = [];
            $fileFields = ['sslc_certificate', 'passport_photo', 'adhar_front', 'adhar_back', 'signature'];
            
            // Add plustwo_certificate only if it's uploaded
            if ($request->hasFile('plustwo_certificate')) {
            ...
            $studentDetail = LeadDetail::create([
                ...
                'course_id' => 8, // Hotel Management course ID
            ...
            try {
                MailService::sendStudentRegistrationEmail($studentDetail, 'Hotel Management');
            } catch (\Exception $e) {
                // Log error but don't fail the registration
                \Log::error('Email sending failed for Hotel Management registration: ' . $e->getMessage());
            }
            ...
            try {
                ...
                // Log error but don't fail the registration
                \Log::error('Failed to create lead activity for Hotel Management registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.hotel-mgmt.register.success', $request->lead_id)
            ]);
