...
{
    public function showWebDevForm($leadId = null)
    {
        ...
        if ($leadId) {
            ...
            if ($lead && $lead->webDevStudentDetails) {
                return view('public.web-dev-registration-success');
            }
        }
        
        // Get Web Development & Designing course subjects (course_id = 13)
        $subjects = Subject::where('course_id', 13)->where('is_active', true)->get();
        
        // Get Web Development & Designing course batches (course_id = 13)
        $batches = Batch::where('course_id', 13)->where('is_active', true)->get();
        
        // Get course data
        $course = \App\Models\Course::find(13);
        
        // Get class times for course_id = 13 (Web Development) if course needs_time
        $classTimes = collect();
        if ($course && $course->needs_time) {
            $classTimes = ClassTime::where('course_id', 13)->where('is_active', true)->get();
        }
        ...
        return view('public.web-dev-registration', compact('subjects', 'batches', 'lead', 'countryCodes', 'course', 'classTimes', 'offlinePlaces'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $studentDetail = LeadDetail::create([
                ...
                'course_id' => 13, // Web Development & Designing course ID
            ...
            try {
                ...
                // Log error but don't fail the registration
                \Log::error('Failed to create lead activity for Web Development registration: ' . $e->getMessage());
            }
            ...
            return response()->json([
                ...
                'data' => $studentDetail,
                'redirect' => route('public.lead.web-dev.register', $request->lead_id)
            ]);
