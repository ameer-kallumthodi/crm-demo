        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="page-header-title">
                    <h5 class="m-b-10">Digital Marketing Converted Leads Management</h5>
                </div>
            </div>
            <div class="col-md-6">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <a href="{{ route('admin.digital-marketing-converted-leads.index') }}" class="btn btn-primary active">
...
<div class="row mb-3">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.digital-marketing-converted-leads.index') }}" id="filterForm">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-12 col-sm-6 col-md-2">
                            ...
                            <a href="{{ route('admin.digital-marketing-converted-leads.index') }}" class="btn btn-secondary" id="digitalMarketingClearFilters">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5>Digital Marketing Converted Leads</h5>
            </div>
            <div class="card-body">
                <!-- Desktop Table View -->
                <div class="d-none d-lg-block">
                    <div class="table-responsive">
                        <table class="table table-hover" id="digitalMarketingTable">
                            <thead>
                                <tr>
                                    <th>SL No</th>
                                    <th>Academic</th>
                ...
                <div class="d-lg-none">
                    ...
                    <div class="text-center py-5">
                        <div class="text-muted">
                            ...
                            <h5>No Digital Marketing converted leads found</h5>
...
@php
$digitalMarketingConvertedLeadsColumns = [
...
if (\App\Helpers\RoleHelper::is_admin_or_super_admin() || \App\Helpers\RoleHelper::is_admission_counsellor()) {
    $digitalMarketingConvertedLeadsColumns[] = ['data' => 'parent_phone', 'name' => 'parent_phone', 'orderable' => false, 'searchable' => false];
}
$digitalMarketingConvertedLeadsColumns = array_merge($digitalMarketingConvertedLeadsColumns, [
    ...
    ['data' => 'class_information', 'name' => 'class_information', 'orderable' => false, 'searchable' => false],
    ['data' => 'orientation_class_status', 'name' => 'orientation_class_status', 'orderable' => false, 'searchable' => false],
    ['data' => 'class_starting_date', 'name' => 'class_starting_date', 'orderable' => false, 'searchable' => false],
    ['data' => 'class_ending_date', 'name' => 'class_ending_date', 'orderable' => false, 'searchable' => false],
    ['data' => 'whatsapp_group_status', 'name' => 'whatsapp_group_status', 'orderable' => false, 'searchable' => false],
    ['data' => 'class_status', 'name' => 'class_status', 'orderable' => false, 'searchable' => false],
    ['data' => 'complete_cancel_date', 'name' => 'complete_cancel_date', 'orderable' => false, 'searchable' => false],
...
<style>
    ...
    #digitalMarketingTable thead th,
    #digitalMarketingTable tbody td {
        white-space: nowrap;
    }
    ...
    #digitalMarketingTable tbody tr:hover {
        background: #fafbff;
    }

    #digitalMarketingTable td .display-value {
        display: inline-block;
        max-width: 220px;
        overflow: hidden;
        text-overflow: ellipsis;
    ...
    #digitalMarketingTable .btn-group .btn {
        margin-right: 4px;
    }

    #digitalMarketingTable .btn-group .btn:last-child {
        margin-right: 0;
    }
</style>
@endpush
...
<script>
    $(document).ready(function() {
        function getDigitalMarketingFilterParams() {
            return {
                ...
                class_information: $('#class_information').val() || '',
                orientation_class_status: $('#orientation_class_status').val() || '',
                whatsapp_group_status: $('#whatsapp_group_status').val() || '',
                class_status: $('#class_status').val() || '',
                programme_type: $('#programme_type').val() || ''
            };
        }

        function updateDigitalMarketingUrlWithFilters() {
            const params = new URLSearchParams(getDigitalMarketingFilterParams());
            const next = `${window.location.pathname}?${params.toString()}`;
            window.history.replaceState({}, '', next);
        }

        const configEl = document.getElementById('digitalMarketingConvertedLeadsConfig');
        const columnsEl = document.getElementById('digitalMarketingConvertedLeadsColumnsData');
        let digitalMarketingTable = null;
        if (configEl && columnsEl && $.fn.DataTable) {
            const ajaxUrl = configEl.dataset.url;
            const columns = JSON.parse(columnsEl.textContent || '[]');
            digitalMarketingTable = $('#digitalMarketingTable').DataTable({
                processing: true,
                serverSide: true,
                searching: true,
                ordering: false,
                ...
                ajax: {
                    ...
                    type: 'GET',
                    data: function(d) {
                        Object.assign(d, getDigitalMarketingFilterParams());
                    }
                },
                columns: columns
            });
        }
        window.reloadDigitalMarketingTable = function(resetPaging = false) {
            if (digitalMarketingTable) {
                digitalMarketingTable.ajax.reload(null, resetPaging);
            }
        };
        ...
        // On batch change load admission batch options
        ...
        $('#filterForm').on('submit', function(e) {
            e.preventDefault();
            updateDigitalMarketingUrlWithFilters();
            window.reloadDigitalMarketingTable(true);
        });
        $('#digitalMarketingClearFilters').on('click', function(e) {
            e.preventDefault();
            $('#filterForm')[0].reset();
            loadAdmissionBatchesByBatch('', '');
            updateDigitalMarketingUrlWithFilters();
            window.reloadDigitalMarketingTable(true);
        });
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (field === 'programme_type') {
                            setTimeout(() => {
                                window.reloadDigitalMarketingTable(false);
                            }, 500);
                        }
