...
@push('styles')
<style>
    td .course-flag-field {
        position: relative;
        min-width: 100px;
        overflow: visible;
    }

    td .course-flag-field.editing > .course-flag-edit-btn {
        display: none !important;
    }

    .course-flag-field.inline-edit .flag-edit-form {
        display: none !important;
        position: absolute;
        top: 100%;
        left: 0 !important;
    ...
    .course-flag-field.inline-edit.editing .flag-edit-form {
        display: block !important;
    }

    .course-flag-field .flag-edit-form .flag-edit-row {
        display: flex;
        align-items: flex-start;
        gap: 8px;
    }

    .course-flag-field .flag-edit-form .flag-select-wrap {
        flex: 1;
        min-width: 0;
    }

    .course-flag-field .flag-edit-form .select2-container {
        width: 100% !important;
        display: block;
    }

    .course-flag-field .flag-edit-form .select2-selection--single {
        height: 36px;
        border: 1px solid #ced4da;
        border-radius: 4px;
    }

    .course-flag-field .flag-edit-form .select2-selection__rendered {
        line-height: 34px;
        padding-left: 8px;
        padding-right: 28px;
    }

    .course-flag-field .flag-edit-form .select2-selection__arrow {
        height: 34px;
    }

    .course-flag-field .flag-edit-form .flag-edit-actions {
        display: flex;
        flex-direction: row;
        flex-shrink: 0;
        align-items: center;
    ...
    .course-flag-field .flag-edit-form .flag-edit-actions .btn {
        padding: 5px 12px;
        font-size: 12px;
        white-space: nowrap;
    }

    .course-flag-field .flag-edit-form .select2-dropdown {
        border-radius: 4px;
    }

    .course-flag-option .course-flag-option-title {
        font-weight: 600;
        font-size: 13px;
        line-height: 1.3;
        color: #212529;
    ...
    .course-flag-option .course-flag-option-desc {
        font-size: 12px;
        line-height: 1.4;
        color: #6c757d;
        white-space: normal;
    ...
    .select2-container--default .select2-results__option--highlighted .course-flag-option .course-flag-option-title,
    .select2-container--default .select2-results__option--highlighted .course-flag-option .course-flag-option-desc {
        color: #fff !important;
    }
    ...
    .course-flag-field .flag-edit-form .select2-container--open .select2-dropdown {
        width: 100% !important;
        left: 0 !important;
        min-width: 0;
    }

    .course-flag-field .flag-edit-form .select2-container--open .select2-dropdown--below {
        margin-top: 4px;
        border: 1px solid #ced4da;
        border-radius: 4px;
    }
...
<script>
(function() {
    const courseUpdateUrlTemplate = @json($courseUpdateUrl ?? '');

    function getCourseFlagUpdateUrl(id) {
        if (courseUpdateUrlTemplate) {
            return courseUpdateUrlTemplate.replace('__ID__', id);
        }
    ...
    function formatFlagOption(state) {
        ...
        return $(
            ...
            + `<div class="course-flag-option-title">${escapeHtml(title)}</div>`
            + (description ? `<div class="course-flag-option-desc">${escapeHtml(description)}</div>` : '')
            + '</div></div></div>'
        );
    }
    ...
    function initFlagSelect2($select) {
        ...
        $select.select2({
            ...
            width: '100%',
            placeholder: 'Select Course Flag',
            allowClear: true,
                ...
                <div class="flag-edit-row">
                    <div class="flag-select-wrap">
                        <select class="form-select form-select-sm flag-select-edit">
                            <option value="">Loading course flags...</option>
                        </select>
                    </div>
                    <div class="flag-edit-actions">
                        <button type="button" class="btn btn-success btn-sm course-flag-save-edit">Save</button>
                        <button type="button" class="btn btn-secondary btn-sm course-flag-cancel-edit">Cancel</button>
                    </div>
                </div>
            </div>
        `;
    ...
    function loadFlagsIntoSelect($select, currentId) {
        $.get('/api/course-flags').done(function(flags) {
            ...
            $select.html('<option value="">Error loading course flags</option>');
            destroyFlagSelect2($select);
            initFlagSelect2($select);
        });
    }
    ...
    $(document).on('click', '.course-flag-field .course-flag-edit-btn', function(e) {
        e.preventDefault();
        e.stopImmediatePropagation();

        const container = $(this).closest('.course-flag-field');
        if (container.hasClass('editing')) {
            return;
        }

        $('.course-flag-field.editing').not(container).each(function() {
            closeFlagEdit($(this));
        });
    ...
    $(document).on('click', '.course-flag-cancel-edit', function(e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        closeFlagEdit($(this).closest('.course-flag-field'));
    });

    $(document).on('click', '.course-flag-save-edit', function(e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        ...
        $.ajax({
            url: getCourseFlagUpdateUrl(id),
            method: 'POST',
            data: {
                field: 'course_flag_id',
                value: value,
